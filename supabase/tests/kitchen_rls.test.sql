...
select results_eq($$select column_name from information_schema.columns where table_schema='public' and table_name='kitchen_order_signals' order by column_name$$,$$values('changed_at'::name),('location_id'::name),('order_id'::name),('revision'::name)$$,'realtime signal payload is minimal');
