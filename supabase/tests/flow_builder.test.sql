...
select is((select n.name from public.flow_nodes n join public.flow_versions v on v.id=n.flow_version_id join public.flows f on f.id=v.flow_id where f.slug='builder-flow' and n.type='START'),'Comece aqui','automatic start has friendly name');
